...
function validateDecision(value, diagnostics) {
  ...
  if (value.action === 'silent') {
    return { action: 'silent', internalReason, confidence, cooldownSeconds, diagnostics };
  }

  const message = cleanText(value.message, 180);
  if (!message) return silent('Intervention had no usable message.', diagnostics);
  if (/(only \d+ left|limited time|act now|discount|guaranteed|definitely|you qualify|strong case)/i.test(message)) {
    return silent('Intervention failed factual/safety validation.', diagnostics);
  }
  ...
  return {
    ...
    confidence,
    cooldownSeconds,
    experienceHint: cleanText(value.experienceHint, 80) || undefined,
    diagnostics,
  };
}
...
export async function onRequestPost(context) {
  ...
  const allowedActions = Array.isArray(body?.allowedActions)
    ? body.allowedActions.filter((item) => typeof item === 'string').slice(0, 20)
    : [];
  ...
  if (!routed.ok) {
    return json({ ok: true, decision: silent('AI unavailable; deterministic silent fallback.') });
  }
  ...
  const parsed = extractJson(routed.text);
  const decision = validateDecision(parsed, diagnostics);

  if (niche === 'law-firms' && decision.action === 'intervene' && /(sue|case is worth|you will win|legal advice|deadline)/i.test(decision.message)) {
...
export async function onRequestGet() {
  return json({
    ok: true,
    route: '/api/decision',
    contract: 'POST only; returns validated silent/intervene decision',
  });
}
